...
- **首位锁定** — 派发顺序的第一位玩家被锁定，新进的高优先级玩家无法插队到其前面，保证排队公平
- **槽位阻塞提示** — 普通玩家排到首位但被 VIP 专属槽位限制无法进入时，排队界面显示"排在首位，但需等待可用槽位..."而非误导性的"下一个就是你"
- **放行确定性标识** — `/smartqueue status` 详情中，确定能进的下一个玩家标绿色 `>>`，不确定时（取决于哪种玩家离开）候选玩家标黄色 `?`
...
```
每次计时器触发的放行顺序：
  1. 首位锁定              （派发顺序第一位，不可被新进玩家插队）
  2. Staff                （始终最先，不受配额限制）
  3. 优先重连              （游戏中断线重连的玩家，跳过不能进的普通玩家）
  4. 防失衡补放行          （偿还被跳过的普通名额 — 见下文）
  5. 比例交替              （VIP:普通 比例循环）
```
...
4. 无论从哪条路径放行普通玩家（首位锁定、比例循环或防失衡补放），`skippedNormalCount` 均自动减 1
5. 防失衡期间放行 VIP → 计数不变（VIP 不被卡住，但不减少债务）
6. 当 `skippedNormalCount` 归零 → 恢复正常比例放行
7. 队列完全清空时 → `skippedNormalCount` 自动归零

这确保了 VIP 永远不会完全饿死普通玩家 — 每次被跳过的普通放行机会都最终会被偿还，无论普通玩家从哪条路径进入。
...
| `/smartqueue status` | 查看排队状态。**OP 和 staff**（可通过 `staff_see_detailed_status` 配置关闭）看到活跃玩家数、总容量（`X / Y (Z+N)` 格式展示基础上限与 Staff 专属）、放行模式及比例、VIP 专属槽位使用情况、放行确定性标识（绿色 `>>` 确定下一个，黄色 `?` 表示不确定候选——当 VIP 和普通都在排队但某一类型被槽位阻塞时触发）、排队总人数、四个独立队列（Staff / 优先重连 / VIP / 普通）及每个玩家的名字和身份标签。**普通玩家**看到简化版：活跃玩家数、最大容量、排队总人数，以及两个合并队列 — 优先重连队列（断线重返）和普通队列（staff + VIP + 普通合并）— 只显示玩家名，不暴露身份标签。 |
...
| 优先级 | 队列 | 说明 |
|---|---|---|
| 0 | **首位锁定** | 派发顺序第一位的玩家被锁定，新进的高优先级玩家无法插队。能否进入在放行时判断（如普通玩家被 VIP 专属槽位阻塞则跳过，等待槽位释放）。 |
| 1 | **Staff 队列** | Staff 玩家（来自 `smartqueue-staff.toml`）。始终最先放行，排在其他所有队列之前。 |
| 2 | **优先重连队列** | 正在游戏中断线后重连的玩家（`WAS_PLAYING` 重连）。按 FIFO 顺序放行（先重连的先进入）。此队列中的普通玩家若被 VIP 槽位限制则跳过。 |
...
- **OP 和 Staff**（当 `staff_see_detailed_status = true` 时）看到总容量及分解，如 `活跃玩家: 30 / 34 (32+2)`，显示基础上限 32 + Staff 专属 2，以及详细的 Staff 槽位使用情况
...
- 到达第 1 位且能进入时，前方人数变为绿色的"下一个就是你！"
- 到达第 1 位但被槽位限制（如 VIP 专属槽位阻止普通玩家进入）时，显示黄色的"排在首位，但需等待可用槽位..."
- **按 ESC 无效** — 排队界面无法意外关闭
- 点击"离开队列"断开连接并返回标题界面
